...
{
    public partial class FrmPrimThemCanh : Form
    {
        #region Constructor
        public FrmPrimThemCanh()
        {
            InitializeComponent();
        }
        #endregion
        ...
        {
            List<Dinh> xp = Data.Data.graph_Prim.dsDinh.ToList();
            // cbx Dinh Xuat Phat
            cbxDinhXP.DataSource = xp;
            cbxDinhXP.DisplayMember = "Ten";
            cbxDinhXP.ValueMember = "ID";

            List<Dinh> kt = Data.Data.graph_Prim.dsDinh.ToList();
            // cbx Dinh Ket Thuc
        ...
        private void FrmPrimThemCanh_Load(object sender, EventArgs e)
        {
            LoadInitControl();
        }
        #endregion
        ...
        private void btnThemCanh_Click(object sender, EventArgs e)
        {
            if (Check())
            {
                ...
                int cnt = Data.Data.graph_Prim.dsCanh.Count ;
                if (cnt == 0) canh.ID = 1;
                else canh.ID = Data.Data.graph_Prim.dsCanh[cnt - 1].ID + 1;

                canh.IDXP = (int) cbxDinhXP.SelectedValue;
                canh.IDKT = (int) cbxDinhKT.SelectedValue;
                canh.x1 = Data.Data.graph_Prim.dsDinh.Where(p => p.ID == canh.IDXP).FirstOrDefault().x;
                canh.y1 = Data.Data.graph_Prim.dsDinh.Where(p => p.ID == canh.IDXP).FirstOrDefault().y;
                canh.x2 = Data.Data.graph_Prim.dsDinh.Where(p => p.ID == canh.IDKT).FirstOrDefault().x;
                canh.y2 = Data.Data.graph_Prim.dsDinh.Where(p => p.ID == canh.IDKT).FirstOrDefault().y;
                canh.TrongSo = Int32.Parse(txtTrongSo.Text);
                canh.mau = Color.Black;
                canh.LoaiCanh = cbxLoaiCanh.SelectedIndex;

                Data.Data.graph_Prim.dsCanh.Add(canh);
